(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .search-progress-cancel { border: 0; background: transparent; padding: 2px 0; text-decoration: underline; cursor: pointer; opacity: .68; }
      ...
      @media (max-width: 600px) {
        .search-progress { padding: 15px; }
        .search-progress-detail { align-items: flex-start; flex-direction: column; gap: 5px; }
      }
    `;
  ...
  function ensureElement() {
    ...
    el.innerHTML = `
      <div class="search-progress-head">
        <span class="search-progress-stage"></span>
        <span class="search-progress-percent">0%</span>
      </div>
      <div class="search-progress-track" aria-hidden="true">
        <div class="search-progress-fill running"></div>
      </div>
      <div class="search-progress-detail">
        <span class="search-progress-eta">This may take a few moments...</span>
        <button type="button" class="search-progress-cancel">Cancel</button>
      </div>
    `;
  ...
  function setProgress(state) {
    ...
    el.hidden = false;
    el.classList.toggle('search-progress-error', state.status === 'error');
    ...
    if (state.status === 'running') {
      el.querySelector('.search-progress-fill').classList.add('running');
    } else {
      el.querySelector('.search-progress-fill').classList.remove('running');
    }
  }
  ...
  function estimateEta(progress, startedAt) {
    ...
    // Only show an ETA once enough real progress has been observed. This avoids
    // inventing a countdown from an arbitrary fixed duration.
  ...
  async function pollProgress(id, startedAt, signal) {
    while (!signal.aborted) {
      try {
        const res = await originalFetch(`/api/search-progress/${encodeURIComponent(id)}`, {
          cache: 'no-store',
          signal,
        });
        ...
        // A temporary polling failure should not make a healthy search look failed.
      }
  ...
  function addProgressId(input, id) {
  ...
  window.fetch = async function trackedFetch(input, init = {}) {
    ...
    const url = addProgressId(input, id);
    ...
    if (kind === 'ai' && typeof requestInit.body === 'string') {
      try {
        ...
        // The URL still carries progressId, so do not block the search.
      }
    }
    ...
    const searchHandle = {
      cancel: () => {
        ...
        setProgress({
          ...
          detail: 'You can start another search whenever you are ready.',
        });
    ...
    try {
      ...
      if (!response.ok) {
        setProgress({
          status: 'error',
          percent: 0,
          stage: 'Search could not be completed',
          detail: 'Please check the error below and try again.',
        });
      } else if (!cancelled) {
        setProgress({
          status: 'complete',
